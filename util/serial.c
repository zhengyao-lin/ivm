...
_ivm_serial_stringPoolFromFile(ivm_file_t *file)
{
	...
	for (i = table,
		...
		tmp_str = ivm_string_pool_alloc(ret, ivm_string_size(&tmp_head));
...
CLEAN:;
	...
	ivm_string_pool_list_iterator_t siter;
	IVM_STRING_POOL_LIST_EACHPTR(ret, siter) {
		ivm_string_pool_free(IVM_STRING_POOL_LIST_ITER_GET(siter));
	}
						 ...
						 ivm_serial_exec_t *ret)
{
	...
	if (ivm_file_write(file, ret->instrs, sizeof(*ret->instrs), ret->size)
		!= ret->size) {
		MEM_FREE(ret->instrs);
		return IVM_FALSE;
	}
