...
OPCODE_GEN(UNPACK_LIST, "unpack_list", I, 1, {
	...
	_TMP_OBJ1 = STACK_POP();
	RTM_ASSERT(
		IVM_IS_TYPE(_TMP_OBJ1, _STATE, IVM_LIST_OBJECT_T),
...
OPCODE_GEN(INVOKE, "invoke", I, -(IVM_OPCODE_VARIABLE_STACK_INC), {
	...
	if (!IVM_IS_TYPE(_TMP_OBJ1, _STATE, IVM_FUNCTION_OBJECT_T)) {
...
OPCODE_GEN(INVOKE_BASE, "invoke_base", I, -(1 + IVM_OPCODE_VARIABLE_STACK_INC), {
	...
	while (!IVM_IS_TYPE(_TMP_OBJ1, _STATE, IVM_FUNCTION_OBJECT_T)) {
	...
	// RTM_ASSERT(IVM_IS_TYPE(_TMP_OBJ1, _STATE,  IVM_FUNCTION_OBJECT_T),
...
OPCODE_GEN(GROUP_TO, "group_to", N, -1, {
	...
	RTM_ASSERT(IVM_IS_TYPE(_TMP_OBJ2, _STATE, IVM_NUMERIC_T),
